      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: getApiErrorMessage(
            error,
            'I could not respond right now. Please check the backend and Groq API key.'
          ),
        },
      ]);
    } finally {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
      <AnimatePresence>
        {isOpen && (
          <motion.section
            ...
            className="w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"
          >
            <div className="border-b border-slate-200 bg-slate-50 px-5 py-4 text-slate-900">
            ...
            <div className="h-80 space-y-3 overflow-y-auto bg-white px-4 py-4">
            ...
            <form onSubmit={handleSubmit} className="border-t border-slate-200 bg-white p-4">
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  className="max-h-28 min-h-[52px] flex-1 resize-none rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                />

                <button
                  type="submit"
                  disabled={sending || !input.trim()}
                  className="inline-flex h-[52px] w-[52px] items-center justify-center rounded-lg bg-blue-600 text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
      ...
      <motion.button
        ...
        className="inline-flex items-center gap-3 rounded-full bg-slate-900 px-5 py-4 text-sm font-semibold text-white shadow-sm"
      >
